function(doc, req) { 
	...
	var cur = this.jnlp.information; 	
	result += '<information><title>'+cur.title+'</title><vendor>'+cur.vendor+'</vendor><homepage>'+cur.homepage+'</homepage><description kind=\"one-line\">'+cur.description+'</description>';
	...
	result += '   <desktop/>'; 
	result += '   <menu submenu=\"'+this.jnlp.information.title+'\"/>';
	result += ' </shortcut>';
	result += '</information>';
	...
	for (var a in this._attachments) { if (a.endsWith('.jar')) { var main = ''; if (a == this.jnlp['main-jar']) main = 'main=\"true\"'; result += ' <jar href=\"'+a+'\" '+main+'/> '; } } 
	result += '</resources>'; 
	result += '  <application-desc main-class=\"'+this.jnlp['main-class']+'\">'; 
